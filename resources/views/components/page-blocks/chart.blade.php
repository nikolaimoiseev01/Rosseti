        ...
        >
            ...
            @foreach($values as $i => $item)
                @php
                    ...
                    $displayVal = formatChartValue($item['value'] ?? '0', $chartLocale);
                ...
                >
                    <div style="display: flex; align-items: flex-end; gap: {{ ($hasSecondValue || $hasThirdValue) ? '6px' : '0' }}">
                                ...
                                <div style="position: absolute; bottom: 0; left: 50%; transform: translateX(-50%); width: 0; pointer-events: none; z-index: 5;">
                                    <div class="lolli-stem-2" style="height: 0; width: 0; transition: height 1s cubic-bezier(0.25, 1, 0.5, 1); position: relative;">
                                        <div style="position: absolute; bottom: 100%; left: 50%; transform: translateX(-50%); display: flex; flex-direction: column; align-items: center; margin-bottom: 8px;">
                                            <div class="lolli-val {{ $scopeId }}-s-value2" style="font-size: 18px; font-weight: 700; color: {{ $eff2 }}; margin-bottom: 8px; white-space: nowrap; opacity: 0; transform: translateY(10px); transition: opacity 0.5s ease 0.1s, transform 0.5s ease 0.1s">{{ formatChartValue($item['value2'] ?? '0', $chartLocale) }}</div>
                        ...
                        @if($hasThirdValue && $val3 > 0)
                            <div style="display: flex; flex-direction: column; align-items: center">
                                <div class="lolli-val {{ $scopeId }}-s-value3" style="font-size: 18px; font-weight: 700; color: {{ $eff3 }}; margin-bottom: 8px; white-space: nowrap; opacity: 0; transform: translateY(10px); transition: opacity 0.5s ease 0.2s, transform 0.5s ease 0.2s">{{ formatChartValue($item['value3'] ?? '0', $chartLocale) }}</div>
            ...
            @foreach($values as $i => $item)
                ...
                >
                    <div style="display: flex; align-items: flex-end; gap: {{ ($hasSecondValue || $hasThirdValue) ? '3px' : '0' }}">
                        ...
                        @if($hasSecondValue && $val2 > 0)
                            <div style="display: flex; flex-direction: column; align-items: center">
                                <div class="bar-val {{ $scopeId }}-s-value2" style="font-size: 13px; font-weight: 700; color: {{ $eff2 }}; margin-bottom: 8px; opacity: 0; transform: translateY(10px); transition: opacity 0.5s ease 0.9s, transform 0.5s ease 0.9s; white-space: nowrap">{{ formatChartValue($item['value2'] ?? '0', $chartLocale) }}</div>
                        ...
                        @if($hasThirdValue && $val3 > 0)
                            <div style="display: flex; flex-direction: column; align-items: center">
                                <div class="bar-val {{ $scopeId }}-s-value3" style="font-size: 12px; font-weight: 600; color: {{ $eff3 }}; margin-bottom: 8px; opacity: 0; transform: translateY(10px); transition: opacity 0.5s ease 1s, transform 0.5s ease 1s; white-space: nowrap">{{ formatChartValue($item['value3'] ?? '0', $chartLocale) }}</div>
            ...
            <svg viewBox="0 0 {{ $svgW }} {{ $svgH }}" preserveAspectRatio="xMidYMid meet" style="width: 100%; display: block; overflow: visible">
                ...
                @if($hasSecondValue)
                    ...
                    @foreach($points2 as $i => $pt)
                        ...
                        <text class="lc-label" x="{{ $pt['x'] }}" y="{{ $lY }}" text-anchor="middle"
                            style="font-family: Inter, PFDinTextCondPro, sans-serif; font-size: 13px; font-weight: 600; fill: {{ $cc['light'] }}; opacity: 0; transition: opacity 0.5s ease">{{ formatChartValue($values[$i]['value2'] ?? '0', $chartLocale) }}</text>
                    @endforeach
                @endif
                ...
                @foreach($points as $i => $pt)
                    <text class="lc-label" x="{{ $pt['x'] }}" y="{{ $pt['y'] - 14 }}" text-anchor="middle"
                        style="font-family: Inter, PFDinTextCondPro, sans-serif; font-size: 15px; font-weight: 700; fill: {{ $cc['text'] }}; opacity: 0; transition: opacity 0.5s ease">{{ formatChartValue($values[$i]['value'] ?? '0', $chartLocale) }}</text>
                @endforeach
